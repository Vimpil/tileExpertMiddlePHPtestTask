...
This is a Symfony 7.2 application that provides endpoints for retrieving tile prices, managing orders, and searching orders with Manticore Search. The application is containerized with Docker Compose and includes comprehensive PHPUnit tests, a MySQL database, and Manticore Search integration.

## 🎯 For MLH Reviewers

**Quick verification (requires Docker):**
```bash
docker compose run --rm --no-deps php php vendor/bin/phpunit --testdox
```

**Expected result:** 11 tests, 65 assertions, all passing in <4s.

**What was accomplished:** Test suite stabilization — isolated controllers with mocked services, removed kernel dependencies, added negative-path coverage. See [`PORTFOLIO_SUMMARY.md`](PORTFOLIO_SUMMARY.md) for full technical details and impact.

**Price endpoint hardening:** The `/price` endpoint now reads its upstream URL and timeout from `PRICE_BASE_URL` and `PRICE_HTTP_TIMEOUT`, and it returns a controlled JSON fallback when the external provider is unavailable instead of a 503.
...
- **Reference:** An improved database schema is included in `docker/mysql/improved_schema.sql` for informational purposes only (not used by the application).

**Note:** The project is usually run with the `test` profile. Docker Compose reads `.env` for container settings, and Symfony test runtime reads `.env.test`.

## Testing Strategy

The default PHPUnit suite is designed to be deterministic and portfolio-friendly:

- External services such as Doctrine, Manticore Search, and database privileges are mocked in controller tests.
- Core controller behavior is covered through success and failure paths, including validation failures.
- You can run the suite locally without relying on a live Manticore instance or populated database.
...
If you prefer running on the host machine, use PHP 8.3+ because PHPUnit 12 is required.

For a focused controller run:

```sh
docker compose run --rm --no-deps php php vendor/bin/phpunit tests/Controller/OrderControllerTest.php
docker compose run --rm --no-deps php php vendor/bin/phpunit tests/Controller/PriceControllerTest.php
```
## Prerequisites

- Docker and Docker Compose
- Git
- Make is not required for this repository
...
By default, the repository includes `.env` and `.env.test` files.  
Docker Compose reads `.env` for container port mappings, and Symfony test runtime reads `.env.test`.

Edit `.env` and `.env.test` to set ports and database credentials:
...
PRICE_BASE_URL=https://tile.expert/fr/tile
PRICE_HTTP_TIMEOUT=10
...
- Your PHP version matches the project requirements (PHP 8.3+ recommended; Composer requires >=8.2 and this project uses PHPUnit 12)
...
```sh
# Windows PowerShell users: use curl.exe because curl is an alias for Invoke-WebRequest.
curl.exe http://localhost:9308/json/search -d '{"index":"orders","query":{"match_all":{}},"limit":5}'
```

## Running the Application

The application is accessible at [http://localhost:8080](http://localhost:8080) (default: `APP_PORT=8080`).

`phpMyAdmin` is available at [http://localhost:8081](http://localhost:8081) (default: `PHPMYADMIN_PORT=8081`).
...
- **Configuration:**
  - `PRICE_BASE_URL` controls the external tile expert URL.
  - `PRICE_HTTP_TIMEOUT` controls the upstream request timeout in seconds.
- **Example Response:**
  ```json
  {
    "price": "56.65",
  ...
  ```
- **Fallback response when the provider is unavailable:**
  ```json
  {
    "price": null,
    "factory": "cobsa",
...
> **Note:**  
> The file `docker/mysql/improved_schema.sql` contains an improved version of the database schema.
...
- **Database Permissions:**
  ...
  # Test profile
  docker compose exec db_test mysql -u root -proot -e "GRANT ALL PRIVILEGES ON test.* TO 'user'@'%'; FLUSH PRIVILEGES;"
  ```
  Or use:
  ```sh
